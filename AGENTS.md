...
- **Run**: `yarn run-worker` (requires RabbitMQ + config)
- **Dependencies**: `cd tools/bouncer_worker && yarn install`

### C++ Build

- **Build system**: CMake >= 3.11, C++20
- **Required dependencies**: Boost, MongoDB CXX Driver (v3.11+), ASSIMP, IfcOpenShell, OpenCASCADE, Eigen, GMP, MPFR
...
- The C++ build requires a **specific custom-built IfcOpenShell** that matches the bouncer's API. The public IfcOpenShell v0.8.0 release has an incompatible `Iterator` constructor signature. The matching build comes from the private `3drepo/tests` repo (archive `IfcOpenShell-0.8.0.tgz`). Without access to that repo, C++ compilation of `ifcUtils/` will fail.
